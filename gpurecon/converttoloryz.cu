...
__global__ void convertoloryz(short* dev_lor_data_array, int* dev_indexxmax, CUDAlor* lines, int nlines, int noffset)
{
	for (int i = threadIdx.x + blockIdx.x * blockDim.x; i < nlines; i += blockDim.x * gridDim.x) {
		...
		rsectorID1 = *(dev_lor_data_array + 6 * dev_indexxmax[(i + noffset)] + 0);
		moduleID1 = *(dev_lor_data_array + 6 * dev_indexxmax[(i + noffset)] + 1);
		crystalID1 = *(dev_lor_data_array + 6 * dev_indexxmax[(i + noffset)] + 2);
		rsectorID2 = *(dev_lor_data_array + 6 * dev_indexxmax[(i + noffset)] + 3);
		moduleID2 = *(dev_lor_data_array + 6 * dev_indexxmax[(i + noffset)] + 4);
		crystalID2 = *(dev_lor_data_array + 6 * dev_indexxmax[(i + noffset)] + 5);
...
__global__ void convertoloryz_ac(short* dev_lor_data_array, int* dev_indexxmax, CUDAlor* lines, float* linesyz_attvalue_list, int nlines, int noffset)
{
	for (int i = threadIdx.x + blockIdx.x * blockDim.x; i < nlines; i += blockDim.x * gridDim.x) {
		...
		CUDAlor* this_line = (CUDAlor*)lines + i;
		float att_value_for_this_line = linesyz_attvalue_list[noffset + i];
